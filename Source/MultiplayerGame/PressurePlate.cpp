...
APressurePlate::APressurePlate()
{
	...
	//turns on replication for the pressure plate actor
	bReplicates = true;
	SetReplicateMovement(true);

	Activated = false;

	//Creates new USceneComponent and stores it inside of the RootComp variable
	RootComp = CreateDefaultSubobject<USceneComponent>(TEXT("RootComp"));
	//Calls SetRootComponent function. Sets RootComp as the root component of the actor
	SetRootComponent(RootComp);

	
	//BELOW IS CODE FOR THE TRIGGER MESH, WHICH IS USED TO TELL IF SOMETHING IS STOOD ON TOP OF THE ACTUAL PRESSURE PLATE MESH

	TriggerMesh = CreateDefaultSubobject<UStaticMeshComponent>(TEXT("TriggerMesh"));
	//Attaches TriggerMesh component to the root component: RootComp
	TriggerMesh->SetupAttachment(RootComp);
	//Turns on replication for the TriggerMesh component
	TriggerMesh->SetIsReplicated(true);

	//auto = special keyword that infers the type you want to use
	//FObjectFinder finds the specified object
	...
	//checks if TriggerMeshAsset succeeded in finding the asset
	if (TriggerMeshAsset.Succeeded())
	{
		//If successful, sets the static mesh using the object found by TriggerMeshAsset
		TriggerMesh->SetStaticMesh(TriggerMeshAsset.Object);
		//Sets the scale and location of the trigger mesh
		TriggerMesh->SetRelativeScale3D(FVector(3.3f, 3.3f, 0.2f));
		TriggerMesh->SetRelativeLocation(FVector(0.0f, 0.0f, 10.0f));
	}


	//DOES THE SAME AS THE ABOVE CODE, BUT NOW FOR THE ACTUAL MESH OF THE PRESSURE PLATE
	...
	//Sets the time to move the pressure plate when stepped on (activated) to 0.25 seconds
...
void APressurePlate::BeginPlay()
{
	...
	//TriggerMesh is only to detect collision. Does not need to be visible.
	TriggerMesh->SetVisibility(false);
	//Only needs to detect collision. Can't block other actors
	TriggerMesh->SetCollisionProfileName(FName("OverlapAll"));

	//Sets the distance the pressure plate moves when activated to 10cm below its starting location
